...
export class PokemonService {
  ...
  constructor(
    ...
    this.defaulLimit = configService.get<number>('default_limit');
  }
  ...
  async findOne(term: string) {
      ...
      if(!pokemon){
        throw new NotFoundException(`No se encontró ningun pokemon con el termino de busqueda ${term}`);
      }
      return pokemon
  }
  ...
  private handleExceptions(error:any){
    ...
    if (error.code === 11000){
      ...
      console.log(error);
      throw new InternalServerErrorException(`No se pudo crear el pokemon - revisar el log`);
    }
  }
}
